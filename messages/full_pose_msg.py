...
class FullPose(MAVMessage):
    ...
    def pose_callback(self, msg):
        ...
        if abs(self.attitude.timestamp - self.local_position.timestamp) > 0.1:
            print(f'Warning: Discarding pose since Attitude({self.attitude.get_hz()}hz) and Local Position({self.local_position.get_hz()}hz) timestamps differ by more than 100 ms.')
            return
        self.update_timestamp(np.average([self.attitude.timestamp, self.local_position.timestamp]))
